...
module.exports.get = middy(async (event, context) => {
...
module.exports.addProduct = middy(async (event, context) => {
...
module.exports.deleteProduct = middy(async (event, context) => {
  const id = Number(event.pathParameters.productId);
  const product = await productRepository.getProduct(id);
  if (!product) {
    throw new httpErrors.NotFoundError('Product not found');
...
module.exports.updateProduct = middy(async (event, context) => {
  const id = Number(event.pathParameters.productId);
  const quantity = event.body ? Math.floor(Number(event.body.quantity)) : 0;
  console.log(event.body.quantity);
  const product = await productRepository.getProduct(id);
  if (!product) {
    throw new httpErrors.NotFoundError('Product not found');
  }
  ...
  .use(validator(addProductSchema))
  .use(httpErrorHandler());
module.exports.clear = middy(async (event, context) => {
  const basket = await clearBasketCommand.execute(event.pathParameters.checkoutID);
  const resource = mapper.map(basket);
  return createResponse(200, resource);
}).use(cors());
module.exports.reset = middy(async (event, context) => {
